  ...
  return (
    <div className="space-y-5 animate-in fade-in duration-500">
      ...
      <Card className="border-border bg-card text-card-foreground">
        <CardHeader className="pb-2"><CardTitle className="text-sm text-muted-foreground">Occupancy Heatmap (Time of Day vs Day of Week)</CardTitle></CardHeader>
        <CardContent>
          <div className="space-y-1">
            <div className="flex">
              <div className="w-12 shrink-0" />
              {HEATMAP_DATA[0].hours.map(h => (
                <div key={h.hour} className="flex-1 text-center text-[9px] text-muted-foreground/50">{h.hour > 12 ? `${h.hour - 12}P` : h.hour === 12 ? "12P" : `${h.hour}A`}</div>
              ))}
            </div>
            {HEATMAP_DATA.map(row => (
              <div key={row.day} className="flex items-center">
                <div className="w-12 shrink-0 text-[10px] text-muted-foreground">{row.day}</div>
                {row.hours.map(h => (
                  <div
                    key={h.hour}
                    ...
                    style={{
                      background: h.occupancy > 80 ? "hsla(18, 100%, 50%, 0.7)" : h.occupancy > 60 ? "hsla(18, 100%, 50%, 0.4)" : h.occupancy > 40 ? "hsla(18, 100%, 50%, 0.2)" : "hsla(0,0%,50%,0.08)",
                    }}
                  />
                ))}
              </div>
          ...
          <div className="flex items-center gap-3 mt-3 text-[10px] text-muted-foreground">
            <span className="flex items-center gap-1"><span className="h-3 w-3 rounded-sm" style={{ background: "hsla(0,0%,50%,0.08)" }} />&lt;40%</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <Card className="border-border bg-card text-card-foreground">
          <CardHeader className="pb-2"><CardTitle className="text-sm text-muted-foreground">Monthly Occupancy Trend</CardTitle></CardHeader>
          <CardContent>
            <ResponsiveContainer width="100%" height={220}>
              <AreaChart data={OCCUPANCY_TREND}>
                ...
                <Tooltip contentStyle={{ background: "hsl(var(--card))", border: "1px solid hsl(var(--border))", borderRadius: 8, color: "hsl(var(--card-foreground))", fontSize: 12 }} />
        ...
        <Card className="border-border bg-card text-card-foreground">
          <CardHeader className="pb-2"><CardTitle className="text-sm text-muted-foreground">Revenue by Room Type</CardTitle></CardHeader>
          <CardContent>
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={ROOM_TYPE_REVENUE}>
                ...
                <Tooltip contentStyle={{ background: "hsl(var(--card))", border: "1px solid hsl(var(--border))", borderRadius: 8, color: "hsl(var(--card-foreground))", fontSize: 12 }} formatter={(v: number) => [`₹${v.toLocaleString("en-IN")}`, "Revenue"]} />
      ...
      {/* Insights */}
      <Card className="border-border bg-card text-card-foreground">
        <CardHeader className="pb-2"><CardTitle className="text-sm text-muted-foreground flex items-center gap-2"><Lightbulb className="h-4 w-4" />Auto-Generated Insights</CardTitle></CardHeader>
        <CardContent className="space-y-2">
          {ANALYTICS_INSIGHTS.map(insight => (
            <div key={insight.id} className="flex items-start gap-3 rounded-lg p-3" style={{
              ...
              <p className="text-xs text-foreground/60">{insight.text}</p>
            </div>
          ))}
        </CardContent>
      </Card>
