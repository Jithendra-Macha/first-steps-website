    ...
    <div className="space-y-5 animate-in fade-in duration-500">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Revenue breakdown and payout history</p>
        </div>
      ...
      <Card className="border-border bg-card text-card-foreground">
        <CardHeader className="pb-2"><CardTitle className="text-sm text-muted-foreground">Revenue vs Net Payout</CardTitle></CardHeader>
        <CardContent>
          <ResponsiveContainer width="100%" height={260}>
            <AreaChart data={MONTHLY_REVENUE_CHART}>
              ...
              <CartesianGrid stroke="hsla(0,0%,50%,0.1)" strokeDasharray="3 3" />
              ...
              <Tooltip contentStyle={{ background: "hsl(var(--card))", border: "1px solid hsl(var(--border))", borderRadius: 8, color: "hsl(var(--card-foreground))", fontSize: 12 }} formatter={(v: number) => [`₹${v.toLocaleString("en-IN")}`, ""]} />
      ...
      <Card className="border-border bg-card overflow-hidden">
        <CardHeader className="pb-2"><CardTitle className="text-sm text-muted-foreground">Payout History</CardTitle></CardHeader>
        <Table>
          <TableHeader>
            <TableRow className="border-border hover:bg-transparent">
              <TableHead className="text-muted-foreground text-[11px]">Month</TableHead>
              <TableHead className="text-muted-foreground text-[11px]">Gross Revenue</TableHead>
              <TableHead className="text-muted-foreground text-[11px]">Platform Fee</TableHead>
              <TableHead className="text-muted-foreground text-[11px]">Taxes</TableHead>
              <TableHead className="text-muted-foreground text-[11px]">Net Payout</TableHead>
              <TableHead className="text-muted-foreground text-[11px]">Status</TableHead>
              <TableHead className="text-muted-foreground text-[11px]">Action</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {EARNINGS.map(e => (
              <TableRow key={e.id} className="border-border/50 hover:bg-accent/50">
                <TableCell className="text-xs text-foreground/70">{e.month}</TableCell>
                <TableCell className="text-xs text-foreground/60">₹{e.grossRevenue.toLocaleString("en-IN")}</TableCell>
                <TableCell className="text-xs text-muted-foreground">₹{e.platformFee.toLocaleString("en-IN")}</TableCell>
                <TableCell className="text-xs text-muted-foreground">₹{e.taxes.toLocaleString("en-IN")}</TableCell>
                <TableCell className="text-xs text-foreground/70 font-medium">₹{e.netPayout.toLocaleString("en-IN")}</TableCell>
                <TableCell>
                ...
                <TableCell>
                  {e.status === "paid" && (
                    <Button size="sm" variant="ghost" className="h-7 text-[10px] text-muted-foreground"><Download className="h-3 w-3 mr-1" />Receipt</Button>
                  )}
                </TableCell>
              </TableRow>
            ))}
